	...
	<div class="navbar navbar-inverse navbar-fixed-top navbar-expand">
		...
		<div class="collapse navbar-collapse" id="collapse">
			<ul class="nav navbar-nav">
				<?php 
                    foreach($pagesSearchLogged as $url => $pagesLog) { ?> <!-- Végighaladunk a $pagesSearch tömbön-->
                        <li <?= (($pagesLog == $searchingLog) ? ' class="active"' : '') ?>>
                            <a href="index.php<?= ($url == '/') ? '' : ('?pagesLog=' . $url) ?>">
                            <?=$pagesLog['text'] ?></a>
                        </li>
                <?php } ?>     
			</ul>
...
<!-- Az oldal törzse-->
<div id="pageCont">
    <?php include("./templates/pages/{$searchingLog['fileLogged']}.tpl.php"); ?> <!-- A kiválasztott oldal template oldal feltöltése -->
</div>
